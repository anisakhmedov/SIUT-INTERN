import{c as X,A as de,e as oe,l as ce,r,t as V,j as e,U as te,C as pe,S as ue}from"./index-ulkMmI0r.js";import{C as ge}from"./ui-SEvN8Z7c.js";import{F as I}from"./funnel-BJAONQde.js";import{E as R}from"./eye-DZrJCz5Y.js";import"./calendar-BukBerVy.js";const me=[["path",{d:"M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z",key:"1oefj6"}],["path",{d:"M14 2v5a1 1 0 0 0 1 1h5",key:"wfsgrz"}],["circle",{cx:"10",cy:"12",r:"2",key:"737tya"}],["path",{d:"m20 17-1.296-1.296a2.41 2.41 0 0 0-3.408 0L9 22",key:"wt3hpn"}]],C=X("file-image",me);const xe=[["path",{d:"M21 12a9 9 0 1 1-6.219-8.56",key:"13zald"}]],se=X("loader-circle",xe);const fe=[["path",{d:"M12 3v12",key:"1x0j5s"}],["path",{d:"m17 8-5-5-5 5",key:"7q97r8"}],["path",{d:"M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4",key:"ih7n3h"}]],z=X("upload",fe),he=["image/jpeg","image/png","image/gif","image/webp"],be=15*1024*1024;function ye(s){return s?he.includes(s.type)?s.size>be?"Maximum file size is 15MB.":"":"Only JPEG, PNG, GIF, and WebP images are allowed.":"Please choose an image file."}function ve(s){if(!s)return null;try{return JSON.parse(s)}catch{return null}}function je(s){return s?.message||s?.data?.message||"Upload failed"}function ae(s,d,c,y){return new Promise((T,m)=>{if(!s){m(new Error("Student ID is required."));return}if(!d){m(new Error("Please choose an image file."));return}const A=new FormData;A.append("image",d);const l=new XMLHttpRequest;l.open("PATCH",`${de}/student/${s}/${c}`);const E=oe();E&&l.setRequestHeader("Authorization",`Bearer ${E}`),l.upload.onprogress=p=>{if(!y)return;if(!p.lengthComputable||!p.total){y(0);return}const _=Math.min(100,Math.round(p.loaded/p.total*100));y(_)},l.onload=()=>{const p=ve(l.responseText);if(l.status>=200&&l.status<300){T(p||{});return}if(l.status===401){ce("expired"),m(new Error("Session expired. Please login again."));return}if(l.status===403){m(new Error("You don't have permission to perform this action"));return}m(new Error(je(p)))},l.onerror=()=>{m(new Error("Upload failed"))},l.send(A)})}function we(s,d,c){return ae(s,d,"passport-image",c)}function Ne(s,d,c){return ae(s,d,"medicine-image",c)}function b(s,d=0){return s?._id||s?.id||s?.studentId||`${s?.name||"student"}-${d}`}function $(s,d){const c=s?.[d]||s?.[`${d}Image`]||s?.[`${d}Url`];return c?typeof c=="string"?c:c?.url?c.url:null:null}const J=20;function Fe({students:s=[],search:d="",onStudentUpdated:c}){const[y,T]=r.useState(s),[m,A]=r.useState(!0),[l,E]=r.useState(""),[p,_]=r.useState("all"),[v,G]=r.useState("all"),[j,L]=r.useState("all"),[k,M]=r.useState(()=>b(s[0])),[ie,D]=r.useState(1),[o,q]=r.useState({passport:{loading:!1,progress:0},medicine:{loading:!1,progress:0}});r.useEffect(()=>{if(T(s),s.length===0){M("");return}s.some((a,n)=>b(a,n)===k)||M(b(s[0]))},[s,k]);const B=r.useCallback((t,a,n)=>{const f=[a,n].filter(Boolean).join(": ");if(t==="success"){V.success(f,{duration:3600});return}if(t==="warning"){V.warning(f,{duration:4200});return}V.error(f,{duration:4600})},[]),W=r.useCallback((t,a)=>!!$(t,a),[]),Y=r.useCallback(t=>[t?.name,t?.surname,t?.lastname].filter(Boolean).join(" ").trim(),[]),Z=r.useMemo(()=>Array.from(new Set(y.map(a=>a?.nameFaculty||a?.faculty?.name).filter(Boolean))).sort((a,n)=>a.localeCompare(n)),[y]),x=r.useMemo(()=>{const t=d.trim().toLowerCase(),a=l.trim().toLowerCase(),n=`${t} ${a}`.trim();return y.filter(i=>{const u=i?.nameFaculty||i?.faculty?.name||"",h=W(i,"passport"),U=W(i,"medicine");return p!=="all"&&u!==p||v==="has"&&!h||v==="missing"&&h||j==="has"&&!U||j==="missing"&&U?!1:n?[i?.name,i?.surname,i?.lastname,i?.nameFaculty,i?.faculty?.name,i?.gender,i?.year].filter(Boolean).join(" ").toLowerCase().includes(n):!0}).sort((i,u)=>Y(i).localeCompare(Y(u)))},[Y,W,l,y,j,v,d,p]),F=Math.max(1,Math.ceil(x.length/J)),P=Math.min(ie,F),re=x.slice((P-1)*J,P*J),H=r.useMemo(()=>{let t=0;return l.trim()&&(t+=1),p!=="all"&&(t+=1),v!=="all"&&(t+=1),j!=="all"&&(t+=1),t},[l,j,v,p]),ne=r.useCallback(()=>{E(""),_("all"),G("all"),L("all"),D(1)},[]);r.useEffect(()=>{D(1)},[d,l,p,v,j]);const g=r.useMemo(()=>x.length?x.find((t,a)=>b(t,a)===k)||x[0]:null,[x,k]);r.useEffect(()=>{if(!x.length){k&&M("");return}x.some((a,n)=>b(a,n)===k)||M(b(x[0]))},[x,k]);const K=r.useCallback((t,a)=>{T(n=>{const f=n.map((i,u)=>b(i,u)!==t?i:{...i,...a});if(c){const i=f.find((u,h)=>b(u,h)===t)||null;i&&c(i)}return f})},[c]),Q=r.useCallback(async(t,a)=>{const n=g;if(!n)return;const f=ye(a);if(f){B("error","Validation error",f);return}const i=b(n),u=t==="passport";q(h=>({...h,[t]:{loading:!0,progress:0}}));try{const U=await(u?we:Ne)(i,a,O=>{q(le=>({...le,[t]:{loading:!0,progress:O}}))}),S=U?.image||null;if(!S)throw new Error("Upload failed");K(i,u?{passport:S,passportImage:S}:{medicine:S,medicineImage:S}),q(O=>({...O,[t]:{loading:!1,progress:100}})),B("success","Upload complete",U?.message||`${u?"Passport":"Medicine"} image uploaded successfully.`)}catch(h){B("error","Upload failed",h?.message||"Upload failed")}finally{setTimeout(()=>{q(h=>({...h,[t]:{loading:!1,progress:0}}))},120)}},[B,g,K]),w=$(g,"passport"),N=$(g,"medicine"),ee=r.useCallback(t=>{t&&window.open(t,"_blank","noopener,noreferrer")},[]);return e.jsxs("div",{className:"student-docs-page",children:[e.jsx("style",{children:`
        .student-docs-page {
